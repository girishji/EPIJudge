...
def find_kth_node_binary_tree(tree: BinaryTreeNode, k: int) -> Optional[BinaryTreeNode]:
    def find_node(tree, remain):
        if tree.left:
            if remain > tree.left.size:
                remain -= tree.left.size
            else:
                return find_node(tree.left, remain)
        remain -= 1
        if remain == 0:
            return tree
        return find_node(tree.right, remain)

    return find_node(tree, k)
